...
有文件哈希凭据时，Git自动换行转换也可能改变接收字节。需要保持原字节的文件可用`.gitattributes`固定；接收后核对实际差异，不能直接重盖哈希掩盖问题。Git忽略但复算必需的文件也要明确纳入成果包。
...
下列核验和接收命令在已有项目仓库内运行；首次接收完整bundle且尚无仓库时，先`git clone <bundle文件> <新目录>`，再进入新目录做`git bundle verify`与内容核查。
